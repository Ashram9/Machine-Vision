...
class Rosenbrock(ScalarFunction):
    ...
    def jacobian(self, x):
        ...
        df_dx2 = 200 * (x2 - x1**2)
        derivative = np.array([[df_dx1], [df_dx2]])
    ...
    def hessian(self, x):
        ...
        df_dx1_dx1 = 1200 * x1**2 - 400 * x2 + 2
        df_dx1_dx2 = -400 * x1
        df_dx2_dx1 = -400 * x1
        df_dx2_dx2 = 200
        second_derivative = np.array([[df_dx1_dx1, df_dx1_dx2], [df_dx2_dx1, df_dx2_dx2]])
...
class LogisticRegressionNLL(ScalarFunction):
    def __init__(self, x, y):   # x&y fixed, variable: phi
        self.x = x
        self.y = y
        self.dims = x.shape[0]

    def __call__(self, phi):    # return negative log-likelihood function
        small_number = 1e-15
    ...
    def jacobian(self, phi):
        ...
        activation = phi.T @ self.x
        derivative = np.sum((sigmoid(activation) - self.y) * self.x, axis=1, keepdims=True)
    ...
    def hessian(self, phi):
        ...
        activation = phi.T @ self.x
        second_derivative = sigmoid(activation) * (1 - sigmoid(activation)) * self.x @ self.x.T
