...
class Crossentropy_softmax_layer(object):
    ...
    def forward(self, x, target):
        ...
        y = -np.sum(target *  np.log(self.softmax_output)) / len(target)
        # avoid taking the logarithm of zero, which would result in undefined or infinite values.
        # y = -np.sum(target * np.log(self.softmax_output + 1e-8)) / len(target)
