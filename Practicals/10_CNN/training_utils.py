...
def train(dataloader, model, criterion,optimizer,device='cpu'):
    ...
    for batch in dataloader:
        ...
        # Note: Zero your gradients for every batch!
        optimizer.zero_grad()
...
def validate(dataloader,model,criterion,device='cpu',split='val'):
    ...
    for batch in dataloader:
        ...
        # Find the class indices with the maximum scores
        _, max_class_indices = outputs.max(dim=1)
        predictions.extend(max_class_indices.squeeze().cpu().numpy())
